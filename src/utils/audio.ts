// Simple Web Audio API Synthesizer for educational feedback
...
export function playBubbleSound(muted: boolean = false) {
  if (muted) return;
  try {
    ...
    const startFreq = 400 + Math.random() * 250;
    osc.type = 'sine';
...
export function playWaterDropSound(muted: boolean = false) {
  if (muted) return;
  try {
    const ctx = getAudioContext();
    if (!ctx) return;
...
export function playSuccessChime(muted: boolean = false) {
  if (muted) return;
  try {
    const ctx = getAudioContext();
    if (!ctx) return;
